...
import { getCalendarColor, MEETING_STATUS_LABELS } from "./adminFormatters";
...
function AdminCalendarPage() {
  ...
  useEffect(() => {
    async function loadMeetings() {
      try {
        ...
        setError("לא הצלחנו לטעון את לוח השנה.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const events = useMemo(
    () =>
      meetings.map((meeting) => ({
        ...
        title: `${meeting.mentor.fullName} / ${meeting.mentee.fullName} - ${MEETING_STATUS_LABELS[meeting.status] || meeting.status}`,
  ...
  return (
    <AdminLayout>
      ...
      {meetings.length === 0 ? (
        <AdminEmpty title="אין פגישות בלוח" />
      ) : (
